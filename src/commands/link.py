...
def linkMinecraftAccount(minecraft_username, hypixel_api_key, discord_user_id, discord_username):
    ...
    if mojang_response.status_code != 200:
        return False, None, None, f"Minecraft username **{minecraft_username}** does not exist"
    ...
    if hypixel_response.status_code != 200:
        return False, None, None, f"Failed to fetch data for Minecraft username **{minecraft_username}** `{hypixel_response.status_code}`"

    player_data = hypixel_response.json().get('player')
    if not player_data:
        return False, None, None, f"No data found for Minecraft username **{minecraft_username}**"
    ...
    if guild_response.status_code != 200:
        return False, None, None, f"Failed to fetch guild data for Minecraft username **{minecraft_username}** `{guild_response.status_code}`"
