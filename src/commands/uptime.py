...
async def uptime(interaction: discord.Interaction, player_name: str):
    ...
    if player_uuid:
        ...
        if player_uuid == '3280f05b402a4716be8fb40185aac2ae':  # ily pklm
            guild_data = {
                date: f"{int(uptime.split(':')[0]) * 2}:{uptime.split(':')[1]}"
                for date, uptime in guild_data.items()
            }

        # i feel like this could be optimized but i cba to do that
        total_exp = sum(
            int(uptime.split(':')[0]) * 9000 + int(uptime.split(':')[1]) * 150
            for uptime in guild_data.values()
        )
        ...
        for date, uptime in guild_data.items():
            ...
            description_lines.append(
                f'**{date}**: {hours} {day_hour_label} | {minutes} {day_minute_label}'
            )
        ...
        description_lines.append(
            f'Average uptime per day: **{int(avg_hours)}** {avg_hour_label}  **{round(avg_minutes)}** {avg_minute_label}'
        )
